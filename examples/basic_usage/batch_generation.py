...
def main():
    ...
    for i, template in enumerate(templates, 1):
        ...
        gc_content = gc_fraction(str(template.fwd())) * 100
        cost = template.cost()
        ...
        print(
            f"Cost: {cost:.2f} | GC: {gc_content:.1f}% | "
...
def recommend_best_template(template_data):
    ...
    print(
        f"Best by optimization cost: Template {best_by_cost['template']} (cost: {best_by_cost['cost']:.2f})"
    )
    print(
        f"Best by primer Tm balance: Template {best_by_tm_balance['template']} (ΔTm: {best_by_tm_balance['tm_diff']:.2f}°C)"
    )
    print(
        f"Best by GC content:        Template {best_by_gc['template']} (GC: {best_by_gc['gc_content']:.1f}%)"
    )
