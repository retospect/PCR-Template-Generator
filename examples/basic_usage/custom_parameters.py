...
def analyze_template(template, name):
    ...
    print(f"  Length: {len(template.fwd())} bp")
...
def compare_optimization_efforts():
    ...
    # Show best result
    successful_results = [(name, cost, template) for name, _, cost, template in results if template]
    if successful_results:
        best_name, best_cost, best_template = min(successful_results, key=lambda x: x[1])
...
def demonstrate_parameter_effects():
    ...
    for tm in [50.0, 54.6, 60.0]:
        ...
        if template:
            fwd_tm = MeltingTemp.Tm_NN(str(template.fwd_primer()))
            rev_tm = MeltingTemp.Tm_NN(str(template.rev_primer()))
            print(f"  Target: {tm}°C → Actual: {fwd_tm:.1f}°C/{rev_tm:.1f}°C (Cost: {template.cost():.2f})")
        else:
