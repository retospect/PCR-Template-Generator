...
def main():
    ...
    for delta in probe_deltas:
        ...
        if template:
            ...
            print(f"     Probe Tm: {probe_tm:.1f}°C, Primer Tm: {(fwd_tm + rev_tm) / 2:.1f}°C")
        else:
...
def demonstrate_constraint_effects():
    ...
    constraint_sets = [
        ("Very Easy", {
            'overall_gc_min': 40.0, 'overall_gc_max': 60.0,
            'primer_tm_tolerance': 2.0, 'max_run_length': 5,
            'probe_tm_delta_min': 5.0, 'probe_tm_delta_max': 15.0
        }),
        ("Easy", {
            'overall_gc_min': 45.0, 'overall_gc_max': 55.0,
            'primer_tm_tolerance': 1.0, 'max_run_length': 4,
            'probe_tm_delta_min': 6.0, 'probe_tm_delta_max': 12.0
        }),
        ("Default", {}),  # Use all defaults
        ("Hard", {
            'overall_gc_min': 49.5, 'overall_gc_max': 50.5,
            'primer_tm_tolerance': 0.3, 'max_run_length': 2,
            'probe_tm_delta_min': 8.5, 'probe_tm_delta_max': 9.5
        }),
        ("Very Hard", {
            'overall_gc_min': 49.8, 'overall_gc_max': 50.2,
            'primer_tm_tolerance': 0.1, 'max_run_length': 1,
            'probe_tm_delta_min': 8.9, 'probe_tm_delta_max': 9.1
        })
    ]
