...
class Sequence:
    ...
    def display(self) -> str:
        ...
        lines.append(
            str(self.fwd_primer()) + (" " * template_spacing) + str(self.probe())
        )
    ...
    def cost(self) -> float:
        ...
        rules.append(
            LongRuns(
                sequence=str(self.fwd()),
                max_len=self._max_run_length,
        ...
        rules.append(
            GCContent(
                sequence=str(self.fwd()),
                min_gc=self._overall_gc_min,
                max_gc=self._overall_gc_max,
                note="Overall",
            )
        )
        ...
        for primer in [self.fwd_primer(), self.rev_primer()]:
            ...
            rules.append(
                GCContent(
                    sequence=primer_str,
                    min_gc=self._primer_gc_min,
                    max_gc=self._primer_gc_max,
                    note="Primer",
                )
            )
            ...
            for clamp_length in [5, 11]:
                ...
                rules.append(
                    GCContent(
                        sequence=clamp_seq,
                        min_gc=self._gc_clamp_min,
                        max_gc=self._gc_clamp_max,
                        note="GC Clamp",
                    )
                )
        ...
        for template in [self.fwd(), self.rev()]:
            ...
            rules.append(
                GCContent(
                    sequence=template_str[-1:],
                    min_gc=-1,
                    max_gc=1,
                    note="3' template ends",
                )
            )

            # Primer forcing: bases after primers should not be GC
            primer_forcing_seq = template_str[
                self._primer_length : self._primer_length + 2
            ]
            rules.append(
                GCContent(
                    sequence=primer_forcing_seq,
                    min_gc=-1,
                    max_gc=1,
                    note="Primer Forcing",
                )
        ...
        rules.append(
            GCContent(
                sequence=probe_seq,
                min_gc=self._probe_gc_min,
                max_gc=self._probe_gc_max,
                note="Probe GC",
            )
        )
        rules.append(
            SingleMatchOnly(
                ...
                pattern=probe_seq[-self._unique_end_length :],
        ...
        rules.append(
            SecondaryLimit(
                sequence=str(self.fwd()),
                max_len=self._max_secondary_length,
