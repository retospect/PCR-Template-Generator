...
def check_imports(file_path):
    ...
    try:
        ...
        has_import = (
            "from pcr_template_generator import" in content or
            "import pcr_template_generator" in content
        )
...
def check_docstring(file_path):
    ...
    try:
        ...
        for i, line in enumerate(lines[:5]):
            ...
            if stripped.startswith('#') and ('python' in stripped or 'coding' in stripped):
                continue
...
def main():
    ...
    for py_file in python_files:
        ...
        if py_file.name not in ["__init__.py"] and "sync_examples.py" not in str(py_file):
            has_import = check_imports(py_file)
            if not has_import:
                warnings.append(f"⚠️  {rel_path}: No pcr_template_generator import found")
