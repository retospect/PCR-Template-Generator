...
def extract_package_info(pyproject_data):
    ...
    package_info = {
        ...
        'python_requires': poetry_section.get('dependencies', {}).get('python', '>=3.10'),
        'dependencies': {},
        'dev_dependencies': {}
    }
...
def update_requirements_txt(examples_dir, package_info):
    ...
    for dep_name in dev_deps_to_include:
        if dep_name in package_info['dev_dependencies']:
            ...
            if isinstance(dep_spec, str):
                content.append(f"{dep_name}{dep_spec}           # For running example tests" if dep_name == 'pytest' else f"{dep_name}{dep_spec}          # For code formatting")
